...
class SLRModel(nn.Module):
    def __init__(self, num_classes, c2d_type, conv_type, use_bn=False, hidden_size=1024):
        ...
        if "swin" in c2d_type:
            ...
            print(f"Swin model loaded with hidden size: {hidden_size}")
        ...
        if "mstcn" in self.t_model:
            print(f"Using MSTCN")
            self.conv1d = MSTCN(input_size=hidden_size, hidden_size=1024 if "swin" in self.c2d_type else hidden_size, num_classes=num_classes)
        else:
        ...
        self.temporal_model = BiLSTMLayer(rnn_type='LSTM', input_size=1024 if "swin" in self.c2d_type else hidden_size, hidden_size=1024 if "swin" in self.c2d_type else hidden_size, num_layers=2, bidirectional=True)
        
        self.classifier = NormLinear(1024 if "swin" in self.c2d_type else hidden_size, self.num_classes)
        self.conv1d.fc = NormLinear(1024 if "swin" in self.c2d_type else hidden_size, self.num_classes)
    ...
    def forward(self, x):
        if len(x.shape) == 4:
            ...
            x = x.repeat(1, 20, 1, 1, 1)

        len_x = torch.tensor([x.shape[1]], dtype=torch.long, device=x.device)
