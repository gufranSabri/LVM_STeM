    ...
    def __init__(self, fast_input_size, slow_input_size, hidden_size, conv_type=2, use_bn=False, num_classes=-1):
        ...
        for layer_idx, ks in enumerate(self.kernel_size):
            ...
            if ks[0] == 'P':
                ...
                fast_modules.append(nn.Conv1d(fast_input_sz, self.hidden_size, kernel_size=int(ks[1]), stride=1, padding=0))
                fast_modules.append(nn.BatchNorm1d(self.hidden_size))
                fast_modules.append(nn.ReLU(inplace=True))

                slow_modules.append(nn.Conv1d(slow_input_sz, self.hidden_size, kernel_size=int(ks[1]), stride=1, padding=0))
                slow_modules.append(nn.BatchNorm1d(self.hidden_size))
                slow_modules.append(nn.ReLU(inplace=True))

                main_modules.append(nn.Conv1d(main_input_sz, self.hidden_size, kernel_size=int(ks[1]), stride=1, padding=0))
