...
#         if self.data_type == "video":
#             input_data, label, fi = self.read_video(idx)
#             input_data_hm, _, _ = self.read_video(idx, hm=True)
#             input_data, label = self.normalize(input_data, label)
#             input_data_hm, _ = self.normalize(input_data_hm, label)
#             # input_data, label = self.normalize(input_data, label, fi['fileid'])
#             return input_data, input_data_hm, torch.LongTensor(label), fi["label"], self.inputs_list[idx]['original_info']
#         elif self.data_type == "lmdb":
#             input_data, label, fi = self.read_lmdb(idx)
#             input_data, label = self.normalize(input_data, label)
#             return input_data, torch.LongTensor(label), self.inputs_list[idx]['original_info']
...
#     def read_video(self, index, hm=False):
#         data_type = "fullFrame-256x256px" if not hm else "poseheatmaps-256x256"
#         fi = self.inputs_list[index]
#         if 'phoenix' in self.dataset:
#             img_folder = os.path.join(self.prefix, data_type , fi['folder'])  
#         elif self.dataset == 'CSL':
#             img_folder = os.path.join(self.prefix, f"features/{data_type}/" + fi['folder'] + "/*.jpg")
#         elif self.dataset == 'CSL-Daily':
#             img_folder = os.path.join(self.prefix, fi['folder'])
...
#     def collate_fn(batch):
#         batch = [item for item in sorted(batch, key=lambda x: len(x[0]), reverse=True)]
#         video, video_hm, label, act_label, info = list(zip(*batch))
...
#                 , dim=0)
#                 for vid in video_hm]
#             padded_video_hm = torch.stack(padded_video_hm)
#         else:
#             max_len = len(video[0])
#             video_length = torch.LongTensor([len(vid) for vid in video])
#             padded_video = [torch.cat(
...
#             return padded_video, video_length, padded_label, label_length, act_label, padded_video_hm ,info
