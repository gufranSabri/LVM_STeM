...
def seq_train(loader, model, optimizer, device, epoch_idx, recoder):
    ...
    for k,v in model.named_parameters():
        if "text_encoder" in k and v.requires_grad:
    ...
    for batch_idx, data in enumerate(tqdm_loader):
        ...
        with autocast():
            ...
            loss = model.criterion_calculation(ret_dict, label, label_lgt)# + ret_dict.get('extra_loss', 0)
...
#         vid_hm = device.data_to_device(data[5][:, ::4])
#         optimizer.zero_grad()

#         with autocast():
#             ret_dict = model(vid, vid_lgt, vid_hm)
#             loss = model.criterion_calculation(ret_dict, label, label_lgt)# + ret_dict.get('extra_loss', 0)
