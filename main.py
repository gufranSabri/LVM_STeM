...
# python main.py --device 6 --dataset phoenix2014 --loss-weights Slow=0.25 Fast=0.25 --work-dir ./work_dir/phoenix2014/
...
class Processor():
    def __init__(self, arg):
        ...
        copy_tree('utils', self.arg.work_dir + 'utils')
        copy_tree('swin', self.arg.work_dir + 'swin')
    ...
    def start(self):
        if self.arg.phase == 'train':
            best_dev, best_avg = 100.0, 100.0
            best_epoch = self.arg.optimizer_args['start_epoch'] - 1
            total_time, epoch_time = 0, 0
            ...
            for epoch in range(self.arg.optimizer_args['start_epoch'], self.arg.num_epoch):
                ...
                if eval_model:
                    dev_wer = seq_eval(self.arg, self.data_loader['dev'], self.model, self.device, 'dev', epoch, self.arg.work_dir, self.recoder, self.arg.evaluate_tool)
                    self.recoder.print_log("Dev WER: {:05.2f}%".format(dev_wer))
                    
                    test_wer = seq_eval(self.arg, self.data_loader["test"], self.model, self.device, "test", epoch, self.arg.work_dir, self.recoder, self.arg.evaluate_tool)
            ...
            if self.arg.load_weights is None and self.arg.load_checkpoints is None: print('Please appoint --weights.')
    ...
    def loading(self):
        ...
        if self.arg.load_weights: self.load_model_weights(model, self.arg.load_weights)
        elif self.arg.load_checkpoints: self.load_checkpoint_weights(model, optimizer)
